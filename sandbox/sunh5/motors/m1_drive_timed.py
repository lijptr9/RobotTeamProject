...
def main():
    ...
    while time_s != 0:
        mtr_sp = int(input("Enter a speed (0 to 900 dps): "))
        distance = int(input("Distance to travel (inches):"))
        if mtr_sp == 0 or distance == 0:
            exit()
        time_s = distance / (mtr_sp * 0.012)
        left_motor.run_forever(speed_sp=mtr_sp)
        right_motor.run_forever(speed_sp=mtr_sp)
        time.sleep(time_s)
        left_motor.stop()
